...
program
	...
	.action(async (key, { skipCheck }) => {
		if (
			!skipCheck &&
			getPynths({ network: 'mainnet' }).filter(({ name }) => name === key).length < 1
		) {
			throw Error(
				`Given key of "${key}" does not exist as a pynth in mainnet (case-sensitive). Use --skip-check to skip this check.`
			);
		}
...
program
	...
	.description('Decode a data payload from a Peri Finance contract')
...
program
	.command('pynths')
	.description('Get the list of pynths')
	.option('-n, --network <value>', 'The network to run off.', x => x.toLowerCase(), 'mainnet')
	.option('-k, --key [value]', 'A specific key wanted')
	.option('-z, --use-ovm', 'Target deployment for the OVM (Optimism).')
	.action(async ({ network, useOvm, key }) => {
		const pynthList = getPynths({ network, useOvm });
		console.log(
			JSON.stringify(
				pynthList.map(entry => {
...
program
	.command('tokens')
	.description('Get the list of ERC20 tokens in Peri Finance')
	.option('-n, --network <value>', 'The network to run off.', x => x.toLowerCase(), 'mainnet')
	.option('-z, --use-ovm', 'Target deployment for the OVM (Optimism).')
	.action(async ({ network, useOvm }) => {
