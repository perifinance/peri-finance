...
const DEFAULTS = {
	...
	contractDeploymentGasLimit: 6.9e6, // TODO split out into seperate limits for different contracts, Proxys, Pynths, PeriFinance
	network: 'kovan',
...
const deployShortingRewards = async ({
	...
	for (const { name, rewardsToken } of shortingRewards) {
		...
		const [rewardsTokenAddress] = [rewardsToken].map(token => {
			...
			if (token) {
				...
				// to be a Pynth, and it'll get the existing contract
		...
		const runStep = async opts =>
			performTransactionalStep({
				...
				deployer,
				gasPrice,
				blockscanLinkPrefix: blockscanLinkPrefix,
				ownerActions,
				ownerActionsFile,
	...
	const tableData = deployer.newContractsDeployed.map(({ name, address }) => [
		name,
		address,
		`${blockscanLinkPrefix}/address/${address}`,
	]);
	console.log();
	if (tableData.length) {
