...
module.exports = async ({
	...
	const pynthsRatesToUpdate = pynths
		.map((pynth, i) =>
			Object.assign(
				{
					currentRate: parseUnits((pynthRates[i] || '').toString() || '0').toString(),
					targetRate: exchangeFeeRates[pynth.category],
				},
				pynth
			)
		)
	...
	if (pynthsRatesToUpdate.length) {
		...
		await runStep({
			...
			comment: 'Set the exchange rates for various pynths',
		});
	}
	...
	// 	write: 'setPeriLiquidationPenalty',
	...
	if (SystemSettings.atomicEquivalentForDexPricing && dexEquivalents) {
		for (const { currencyKey, equivalent } of Object.values(dexEquivalents)) {
			await runStep({
				...
				comment:
					'SIP-120 Set the equivalent token - used in uniswap pools - corresponding to this pynth',
			});
		}
	}
	...
	if (SystemSettings.atomicExchangeFeeRate && atomicExchangeFeeRate) {
		for (const [currencyKey, rate] of Object.entries(atomicExchangeFeeRate)) {
			await runStep({
				...
				comment: 'SIP-120 Set the exchange fee rate for swapping atomically into this pynth',
			});
		}
	}
	...
	if (SystemSettings.atomicVolatilityConsiderationWindow && atomicVolatilityConsiderationWindow) {
		for (const [currencyKey, seconds] of Object.entries(atomicVolatilityConsiderationWindow)) {
			await runStep({
				...
				comment: 'SIP-120 Set the atomic volatility window for this pynth (in seconds)',
			});
		}
	}
	...
	if (SystemSettings.atomicVolatilityUpdateThreshold && atomicVolatilityUpdateThreshold) {
		for (const [currencyKey, threshold] of Object.entries(atomicVolatilityUpdateThreshold)) {
			await runStep({
				...
				comment:
					'SIP-120 Set the atomic volatility count for this pynth during the volatility window',
			});
		}
	}
