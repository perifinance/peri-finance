...
module.exports = async ({ network, useOvm, providerUrl, pynths, oldExrates, feeds }) => {
	const output = [];
	const { etherscanUrl } = loadConnections({ network });

	const provider = new ethers.providers.JsonRpcProvider(providerUrl);

	const allFeeds = Object.values(feeds).concat(pynths);

	let abi;

	for (const { name, asset, feed } of allFeeds) {
		const currencyKey = name || asset; // either name of pynth or asset for standalone
		if (feed) {
