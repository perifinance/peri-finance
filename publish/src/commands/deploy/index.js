...
const addPynthsToProtocol = require('./add-pynths-to-protocol');
...
const configurePynths = require('./configure-pynths');
...
const deployPynths = require('./deploy-pynths');
...
const { normalizeHardhatNetworkAccountsConfig } = require('hardhat/internal/core/providers/util.js');
...
const deploy = async ({
	...
	const { pynthsToAdd } = await deployPynths({
		account,
		addressOf,
		addNewPynths,
		config,
	...
	// Configure all feeds as standalone in case they are being used as pynth currency keys (through pynth),
	// or directly (e.g. futures). Adding just one or the other may cause issues if e.g. initially futures
	// market exists, but later a pynth is added. Or if initially both exist, but later the spot pynth
