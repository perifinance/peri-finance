	...
	for (const name of Object.keys(config)) {
		...
		if (result.data.result === 'Contract source code not verified') {
			...
			const warningHeader = () => {
				...
				} else if (config[`Proxy${name}`] || /^Pynth(i|s)/.test(name)) {
					const optionalProxyLink = () => {
						// Pynths source code will be reused by Etherscan, so don't include link to
						// proxy as it may not be the correct one
						if (/^Pynth(s|i)/.test(name)) {
							return '\nThe proxy can be found by looking up the PROXY property on this contract.';
						}
						return (
							`\nThe proxy for this contract can be found here:\n\n` +
							`https://contracts.peri.finance/${network !== 'mainnet' ? network + '/' : ''}${
								name === 'PeriFinance' ? 'ProxyERC20' : 'Proxy' + name
							}`
						);
					};
					return (
