...
const importFeePeriods = async ({
	...
	for (let i = 0; i <= feePeriodLength - 1; i++) {
		...
		if (!skipTimeCheck) {
			if (period.feePeriodId === '0') {
				throw Error(
					`Fee period at index ${i} has NOT been set. Are you sure this is the right FeePool source? ${blockscanLinkPrefix}/address/${sourceContractAddress} `
				);
				...
				throw Error(
					`The initial fee period is more than one week ago - this is likely an error. ` +
						...
						`be the one most recently replaced). Given: ${blockscanLinkPrefix}/address/${sourceContractAddress}`
				);
			}
		}
			...
			if (period.feePeriodId !== '1' && period.startTime !== '0') {
				throw Error(
					`The new target FeePool already has imported fee periods (one or more entries has ` +
						...
						`(this should be the most recently deployed). Given: ${blockscanLinkPrefix}/address/${targetContractAddress}`
				);
			}
		}
	} else {
	...
	for (const feePeriod of feePeriods) {
		...
		console.log(
			green(
				`Successfully emitted importFeePeriod with transaction: ${blockscanLinkPrefix}/tx/${transactionHash}`
			)
		);
	}
