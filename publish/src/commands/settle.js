...
// The block where PeriFinance first had SIP-37 added (when ExchangeState was added)
const fromBlockMap = {
	...
	kovan: 20528323,
	rinkeby: 7100439,
	// Note: ropsten was not settled. Needs to be done after https://github.com/FinancePeri-Team/peri-finance/pull/699
...
const settle = async ({
	...
	if (pynth) {
		console.log(gray('Filtered to pynth:'), yellow(pynth));
	}
	...
	if (balance < '0.1') {
		if (dryRun) {
			...
			console.log(gray(`${blockscanLinkPrefix}/tx/${transactionHash}`));
		}
	}
	...
	const fetchAllEvents = ({ pageSize = 10e3, startingBlock = fromBlock, target }) => {
		const innerFnc = async () => {
			...
			const pageOfResults = await target.getPastEvents('PynthExchange', {
	...
	try {
		...
		exchanges = await fetchAllEvents({ target: PeriFinance });
		saveExchangesToFile({
			network,
			fromBlock,
			exchanges,
	...
	for (const {
		...
		if (+numEntries > 0) {
			...
			const isPynthTheDest = new RegExp(pynth).test(web3.utils.hexToUtf8(toCurrencyKey));
			const isPynthOneSrcEntry = !!fromPynths.find(src => web3.utils.hexToUtf8(src) === pynth);

			// skip when filtered by pynth if not the destination and not any of the sources
			if (pynth && !isPynthTheDest && !isPynthOneSrcEntry) {
				continue;
			}
			...
			if (showDebt) {
				...
				if (reclaimAmount > 0) {
					const pynth = await PeriFinance.methods.pynths(toCurrencyKey).call();

					const Pynth = new web3.eth.Contract(getSource({ contract: 'Pynth' }).abi, pynth);

					const balance = await Pynth.methods.balanceOf(account).call();
...
module.exports = {
	...
	cmd: program =>
		program
			...
			.option('-s, --pynth <pynth>', 'Filter to a specific pynth or regex')
