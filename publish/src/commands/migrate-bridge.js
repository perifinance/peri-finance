...
const migrateBridge = async ({
	...
	const newBridgeAddress = getTarget({ deploymentPath, contract: 'PeriFinanceBridgeToOptimism' });
	...
	// New bridge should not have PERI balance
	// but old bridge should have a positive balance
	const peri = getContract({
	...
	const newBalance = web3.utils.fromWei(
		await peri.methods.balanceOf(newBridgeAddress).call(),
		'ether'
	);
	if (newBalance > 0) {
		throw new Error(
			red(`❌ New bridge already has a positive PERI balance of ${newBalance.toString()} PERI`)
		);
	}
	const oldBalance = web3.utils.fromWei(
		await peri.methods.balanceOf(oldBridgeAddress).call(),
		'ether'
	);
	if (oldBalance === 0) {
	...
	try {
		await confirmAction(
			yellow("⚠⚠⚠ WARNING: You're about to perform the PERI migration. Are you sure?")
		);
	} catch (err) {}
...
module.exports = {
	...
	cmd: program =>
		program
			...
			.description('Migrates peri from an L2 deposit contract into another')
