	...
	describe('Contract setup', () => {
		...
		describe('when setting up new feed ids', () => {
			const feeds = [
				...
				{ assetId: toBytes32('pBTC'), feedId: toBytes32('feed-pBTC') },
			];
	...
	describe('Contract operations', () => {
		...
		describe('when attempting to updatePythPrice with and without payment', () => {
			...
			it('charges the right amount, even if sending more', async () => {
				...
				const beforeUserBalance2 = ethers.BigNumber.from(beforeUserBalance.toString());
				const weiUsedInTx2 = ethers.BigNumber.from(weiUsedInTx.toString());
