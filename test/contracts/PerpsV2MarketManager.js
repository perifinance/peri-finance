...
contract('FuturesMarketManager (PerpsV2)', accounts => {
	...
	before(async () => {
		...
		await pUSD.issue(trader, initialMint, { from: owner });
	});

	addSnapshotBeforeRestoreAfterEach();

	describe('Basic parameters', () => {
		it('Requires pUSD contract', async () => {
			const required = await futuresMarketManager.resolverAddressesRequired();
			assert.deepEqual(required, ['PynthpUSD', 'FeePool', 'Exchanger'].map(toBytes32));
		});
	...
	describe('Market management', () => {
		const currencyKeys = ['pBTC', 'pETH'].map(toBytes32);
		...
		it('Adding a single market', async () => {
			...
			assert.equal(await futuresMarketManager.marketForKey(toBytes32('pLINK')), proxy.address);
		});

		it('Adding multiple markets', async () => {
			const keys = ['pLINK', 'sPERI'].map(toBytes32);
		...
		it('Can add more than one market for the same asset', async () => {
			...
			const secondKey = toBytes32('pETH-2'); // different market key
		...
		it('Removing markets by key', async () => {
			await futuresMarketManager.removeMarketsByKey([toBytes32('pETH')], { from: owner });
			...
			const proxy = await putBehindProxy(market);
			await futuresMarketManager.addProxiedMarkets([proxy.address], { from: owner });
			await futuresMarketManager.removeMarketsByKey(['pBTC', 'pLINK'].map(toBytes32), {
				from: owner,
			});
		...
		it('Cannot remove a market which does not exist', async () => {
			await assert.revert(
				futuresMarketManager.removeMarketsByKey([toBytes32('pLINK')], { from: owner }),
				'Unknown market'
			);
	...
	describe('pUSD issuance', () => {
		...
		it('only markets are permitted to issue or burn pUSD', async () => {
			await onlyGivenAddressCanInvoke({
	...
	describe('Pay fees', () => {
		...
		it('pays fees', async () => {
			...
			assert.bnEqual(await pUSD.balanceOf(FEE_ADDRESS), preBalance.add(toUnit('10')));
		});
	...
	describe('Aggregated Debt', () => {
		it('perpsV2 debt is zero when no markets are deployed', async () => {
			...
			// issue some pUSD
			pUSD.issue(trader, toUnit(100), { from: owner });
		...
		describe('when there are multiple markets', () => {
			...
			const currencyKeys = ['pBTC', 'pETH', 'pLINK'].map(toBytes32);
			let markets, proxies;
			beforeEach(async () => {
				markets = await Promise.all(
					currencyKeys.map(k =>
	...
	describe('Market summaries', () => {
		...
		beforeEach(async () => {
			...
			await pUSD.issue(trader, traderInitialBalance);
		...
		it('All summaries', async () => {
			...
			assert.equal(linkSummary.market, await futuresMarketManager.marketForKey(toBytes32('pLINK')));
