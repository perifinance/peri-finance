...
contract('CollateralErc20', async accounts => {
	...
	it('should set constructor params on deployment', async () => {
		...
		assert.equal(await cerc20.collateralKey(), pBTC);
		assert.equal(await cerc20.pynths(0), toBytes32('PynthpUSD'));
		assert.equal(await cerc20.pynths(1), toBytes32('PynthpBTC'));
	...
	describe('cratio test', async () => {
		describe('pUSD loans', async () => {
			...
			it('when the price falls by 25% our c ratio is 150%', async () => {
				await exchangeRates.updateRates([pBTC], ['7500'].map(toUnit), await currentTime(), {
					from: oracle,
				});
			...
			it('when the price increases by 100% our c ratio is 400%', async () => {
				await exchangeRates.updateRates([pBTC], ['20000'].map(toUnit), await currentTime(), {
					from: oracle,
				});
		...
		describe('pBTC loans', async () => {
			beforeEach(async () => {
				tx = await cerc20.open(twoRenBTC, toUnit(1), pBTC, {
					from: account1,
				});
			...
			it('price changes should not change the cratio', async () => {
				await exchangeRates.updateRates([pBTC], ['75'].map(toUnit), await currentTime(), {
					from: oracle,
				});
	...
	describe('max loan test', async () => {
		it('should convert correctly', async () => {
			// $150 worth of btc should allow 100 pUSD to be issued.
			const pUSDAmount = await cerc20.maxLoan(toUnit(0.015), pUSD);

			assert.bnClose(pUSDAmount, toUnit(100), 100);

			// $150 worth of btc should allow $100 (1) of pETH to be issued.
			const pETHAmount = await cerc20.maxLoan(toUnit(0.015), pETH);

			assert.bnEqual(pETHAmount, toUnit(1));
		});
	});
	...
	describe('liquidation amount test', async () => {
		...
		it('when we start at 200%, a price shock of 30% in the collateral requires 25% of the loan to be liquidated', async () => {
			await exchangeRates.updateRates([pBTC], ['7000'].map(toUnit), await currentTime(), {
				from: oracle,
			});
		...
		it('when we start at 200%, a price shock of 40% in the collateral requires 75% of the loan to be liquidated', async () => {
			await exchangeRates.updateRates([pBTC], ['6000'].map(toUnit), await currentTime(), {
				from: oracle,
			});
		...
		it('when we start at 200%, a price shock of 45% in the collateral requires 100% of the loan to be liquidated', async () => {
			await exchangeRates.updateRates([pBTC], ['5500'].map(toUnit), await currentTime(), {
				from: oracle,
			});
		...
		// 	tx = await cerc20.open(750000000, fiveThousandpUSD, pUSD, {
		// 		from: account1,
		// 	});
		...
		// 	await exchangeRates.updateRates([pBTC], ['9000'].map(toUnit), await currentTime(), {
		// 		from: oracle,
		// 	});
	...
	describe('collateral redeemed test', async () => {
		...
		it('when BTC is @ $10000 and we are liquidating 1000 pUSD, then redeem 0.11 BTC', async () => {
			collateralRedeemed = await cerc20.collateralRedeemed(pUSD, oneThousandpUSD);
		...
		it('when BTC is @ $7000 and we are liquidating 2500 pUSD, then redeem 0.36666 ETH', async () => {
			await exchangeRates.updateRates([pBTC], ['7000'].map(toUnit), await currentTime(), {
				from: oracle,
			});

			collateralRedeemed = await cerc20.collateralRedeemed(pUSD, toUnit(2500));
		...
		it('regardless of BTC price, we liquidate 1.1 * amount when doing pETH', async () => {
			collateralRedeemed = await cerc20.collateralRedeemed(pBTC, toUnit(1));

			assert.bnEqual(collateralRedeemed, toUnit(1.1));

			await exchangeRates.updateRates([pBTC], ['1000'].map(toUnit), await currentTime(), {
				from: oracle,
			});
	...
	describe('opening', async () => {
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling openLoan() reverts', async () => {
						await assert.revert(
							cerc20.open(oneRenBTC, onepUSD, pUSD, { from: account1 }),
					...
					describe(`when ${section} is resumed`, () => {
						...
						it('then calling openLoan() succeeds', async () => {
							await cerc20.open(oneRenBTC, onepUSD, pUSD, {
								from: account1,
							});
						});
					});
			...
			describe('when rates have gone stale', () => {
				...
				describe('when BTC gets a rate', () => {
					...
					it('then calling openLoan() succeeds', async () => {
						await cerc20.open(oneRenBTC, onepUSD, pUSD, { from: account1 });
					});
				});
			});
		});
		...
		describe('revert conditions', async () => {
			...
			it('should revert if they send 0 collateral', async () => {
				await assert.revert(
					cerc20.open(toUnit(0), onepUSD, pUSD, { from: account1 }),
			...
			it('should revert if the requested loan exceeds borrowing power', async () => {
				await assert.revert(
					cerc20.open(oneRenBTC, toUnit(10000), pUSD, {
						from: account1,
					}),
		...
		describe('should open a btc loan denominated in pUSD', async () => {
			const fiveHundredSUSD = toUnit(500);
			let issueFeeRate;
			let issueFee;

			beforeEach(async () => {
				tx = await cerc20.open(oneRenBTC, fiveHundredSUSD, pUSD, {
					from: account1,
				});
		...
		describe('should open a btc loan denominated in pBTC', async () => {
			let issueFeeRate;
			let issueFee;

			beforeEach(async () => {
				tx = await cerc20.open(fiveRenBTC, toUnit(2), pBTC, {
					from: account1,
				});
			...
			it('should issue the correct amount to the borrower', async () => {
				...
				assert.bnEqual(await pBTCPynth.balanceOf(account1), expecetdBalance);
			});
	...
	describe('deposits', async () => {
		beforeEach(async () => {
			tx = await cerc20.open(twoRenBTC, oneHundredpUSD, pUSD, {
				from: account1,
			});
	...
	describe('withdraws', async () => {
		...
		beforeEach(async () => {
			loan = await cerc20.open(twoRenBTC, oneHundredpUSD, pUSD, {
				from: account1,
			});
	...
	describe('repayments', async () => {
			...
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling repay() reverts', async () => {
						await assert.revert(
							cerc20.repay(account1, id, onepUSD, { from: account1 }),
							'Operation prohibited'
						);
					});
					describe(`when ${section} is resumed`, () => {
						...
						it('then calling repay() succeeds', async () => {
							await cerc20.repay(account1, id, onepUSD, { from: account1 });
						});
					});
				});
			});
		...
		describe('revert conditions', async () => {
			...
			it('should revert if they have no pUSD', async () => {
				await assert.revert(
					cerc20.repay(account1, id, tenpUSD, { from: account2 }),
		...
		describe('should allow repayments on an pUSD loan', async () => {
			...
			beforeEach(async () => {
				...
				tx = await cerc20.repay(account1, id, tenpUSD, { from: account2 });
				loan = await state.getLoan(account1, id);
			});
		...
		describe('it should allow repayments on an pBTC loan', async () => {
			const expectedString = '10000';

			beforeEach(async () => {
				tx = await cerc20.open(fiveRenBTC, twoRenBTC, pBTC, {
					from: account1,
				});
				...
				await issuepBTCtoAccount(twoRenBTC, account2);
			...
			it('should work reduce the repayers balance', async () => {
				...
				assert.bnEqual(await pBTCPynth.balanceOf(account2), expectedBalance);
			});
	...
	describe('liquidations', async () => {
		beforeEach(async () => {
			// make a loan here so we have a valid ID to pass to the blockers and reverts.
			tx = await cerc20.open(oneRenBTC, toUnit(5000), pUSD, {
				from: account1,
			});
		...
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling repay() reverts', async () => {
						await assert.revert(
							cerc20.liquidate(account1, id, onepUSD, { from: account1 }),
							'Operation prohibited'
						);
					});
					describe(`when ${section} is resumed`, () => {
						...
						it('then calling liquidate() succeeds', async () => {
							...
							await cerc20.liquidate(account1, id, onepUSD, { from: account1 });
						});
					});
				});
			});
		...
		describe('revert conditions', async () => {
			it('should revert if they have no pUSD', async () => {
				await assert.revert(
					cerc20.liquidate(account1, id, onepUSD, { from: account2 }),
					'Not enough pynth balance'
				);
			});
		...
		describe('should allow liquidations on an undercollateralised pUSD loan', async () => {
			...
			beforeEach(async () => {
				const timestamp = await currentTime();
				await exchangeRates.updateRates([pBTC], ['7000'].map(toUnit), timestamp, {
					from: oracle,
				});

				await issuepUSDToAccount(toUnit(5000), account2);
		...
		describe('when a loan needs to be completely liquidated', async () => {
			beforeEach(async () => {
				const timestamp = await currentTime();
				await exchangeRates.updateRates([pBTC], ['5000'].map(toUnit), timestamp, {
					from: oracle,
				});

				loan = await state.getLoan(account1, id);

				await issuepUSDToAccount(toUnit(10000), account2);
	...
	describe('closing', async () => {
		...
		describe('revert conditions', async () => {
			it('should revert if they have no pUSD', async () => {
				await assert.revert(cerc20.close(id, { from: account1 }), 'Not enough pynth balance');
			});
	...
	describe('drawing', async () => {
		...
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling draw() reverts', async () => {
						await assert.revert(
							cerc20.draw(id, onepUSD, { from: account1 }),
							'Operation prohibited'
						);
					});
					describe(`when ${section} is resumed`, () => {
			...
			describe('when rates have gone stale', () => {
				...
				describe('when BTC gets a rate', () => {
					...
					it('then calling draw() succeeds', async () => {
						await cerc20.draw(id, onepUSD, { from: account1 });
					});
				});
			});
		});
		...
		describe('should draw the loan down', async () => {
			beforeEach(async () => {
				tx = await cerc20.draw(id, oneThousandpUSD, { from: account1 });
