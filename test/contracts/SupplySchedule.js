...
contract('SupplySchedule', async accounts => {
	...
	let supplySchedule, periFinanceProxy, decayRate;
	...
	beforeEach(async () => {
		...
		periFinanceProxy = await setupContract({ accounts, contract: 'ProxyERC20' });
	...
	describe('linking periFinance', async () => {
		it('should have set periFinance proxy', async () => {
			const periFinanceProxy = await supplySchedule.periFinanceProxy();
			assert.equal(periFinanceProxy, periFinanceProxy);
		});
		it('should revert when setting periFinance proxy to ZERO_ADDRESS', async () => {
			await assert.revert(supplySchedule.setPeriFinanceProxy(ZERO_ADDRESS, { from: owner }));
		});

		it('should emit an event when setting periFinance proxy', async () => {
			const txn = await supplySchedule.setPeriFinanceProxy(account2, { from: owner });

			assert.eventEqual(txn, 'PeriFinanceProxyUpdated', {
				newAddress: account2,
			});
		});

		it('should disallow a non-owner from setting the periFinance proxy', async () => {
			await onlyGivenAddressCanInvoke({
				fnc: supplySchedule.setPeriFinanceProxy,
				args: [account2],
				address: owner,
				accounts,
			});
	...
	describe('functions and modifiers', async () => {
		...
		describe.skip('mintable supply', async () => {
			...
			async function checkMintedValues(
				...
				// call updateMintValues to mimic periFinance issuing tokens
			...
			describe('setting weekCounter and lastMintEvent on supplySchedule to week 39', async () => {
				...
				beforeEach(async () => {
					...
					// setup new instance
					await instance.setPeriFinanceProxy(periFinanceProxy.address, { from: owner });
					await periFinanceProxy.setTarget(periFinance, { from: owner });
				});
