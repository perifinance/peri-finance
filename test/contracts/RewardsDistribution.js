...
contract('RewardsDistribution', async accounts => {
	...
	it('should set constructor params on deployment', async () => {
		...
		assert.equal(await instance.periFinanceProxy(), account3);
	...
	describe('when the authority is distributing rewards', async () => {
		beforeEach(async () => {
			...
			// Set the PERI Token Transfer Address
			await rewardsDistribution.setPeriFinanceProxy(periFinance.address, {
				from: owner,
			});
		...
		it('should send the correct amount of tokens to the listed addresses', async () => {
			...
			// Transfer PERI to the RewardsDistribution contract address
			await periFinance.methods['transfer(address,uint256)'](
			...
			const balanceOfRewardsContract = await periFinance.balanceOf(rewardsDistribution.address);
			assert.bnEqual(balanceOfRewardsContract, totalToDistribute);

			// Distribute Rewards called from PeriFinance contract as the authority to distribute
			...
			const balanceOfAccount1 = await periFinance.balanceOf(account1);
			assert.bnEqual(balanceOfAccount1, toUnit('5000'));

			// Check Account 2 balance
			const balanceOfAccount2 = await periFinance.balanceOf(account2);
			assert.bnEqual(balanceOfAccount2, toUnit('10000'));

			// Check Reward Escrow balance
			const balanceOfRewardEscrow = await periFinance.balanceOf(rewardEscrowAddress);
		...
		it('should call the notifyRewardAmount on mockRewardsRecipient', async () => {
			...
			// Transfer PERI to the RewardsDistribution contract address
			await periFinance.transfer(rewardsDistribution.address, totalToDistribute, { from: owner });
