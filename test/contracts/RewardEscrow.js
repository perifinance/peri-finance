...
contract('RewardEscrow', async accounts => {
	...
	let rewardEscrow, periFinance, feePool;
	...
	describe('Constructor & Settings ', async () => {
		it('should set periFinance on contructor', async () => {
			const periFinanceAddress = await rewardEscrow.periFinance();
		...
		it('should allow owner to set periFinance', async () => {
			await rewardEscrow.setPeriFinance(ZERO_ADDRESS, { from: owner });
			const periFinanceAddress = await rewardEscrow.periFinance();
	...
	describe('Functions', async () => {
		...
		describe('Vesting Schedule Writes', async () => {
			it('should not create a vesting entry with a zero amount', async () => {
				...
				await periFinance.transfer(rewardEscrow.address, toUnit('1'), {
					from: owner,
				});
		...
		describe('Vesting Schedule Reads ', async () => {
			beforeEach(async () => {
				...
				await periFinance.transfer(rewardEscrow.address, toUnit('6000'), {
					from: owner,
				});
			...
			it('should append a vesting entry and increase the contracts balance', async () => {
				const balanceOfRewardEscrow = await periFinance.balanceOf(rewardEscrow.address);
		...
		describe('Vesting', async () => {
			...
			it('should vest and transfer peri from contract to the user', async () => {
				...
				assert.bnEqual(await periFinance.balanceOf(rewardEscrow.address), toUnit('0'));
			});
		...
		describe('Stress Test', () => {
			it('should not create more than MAX_VESTING_ENTRIES vesting entries', async () => {
				...
				// Transfer of PERI to the escrow must occur before creating an entry
				await periFinance.transfer(rewardEscrow.address, toUnit('260'), {
					from: owner,
				});
			...
			it('should be able to vest 52 week * 5 years vesting entries', async () => {
				...
				// Check user has all their vested PERI
				assert.bnEqual(await periFinance.balanceOf(account1), toUnit('260'));
			...
			it('should be able to read an accounts schedule of 5 vesting entries', async () => {
				// Transfer of PERI to the escrow must occur before creating an entry
				await periFinance.transfer(rewardEscrow.address, toUnit('5'), {
					from: owner,
				});
