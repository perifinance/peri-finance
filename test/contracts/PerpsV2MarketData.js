...
contract('PerpsV2MarketData', accounts => {
	...
	const newMarketKey = toBytes32('pETH' + keySuffix);
	const newAssetKey = toBytes32('pETH');
	...
	before(async () => {
		...
		for (const symbol of ['pETH', 'sLINK']) {
		...
		// Issue the traders some pUSD
		await pUSD.issue(trader1, traderInitialBalance);
		await pUSD.issue(trader2, traderInitialBalance);
		await pUSD.issue(trader3, traderInitialBalance);
	...
	describe('Market summaries', () => {
		it('For markets', async () => {
			const pETHSummary = (await perpsV2MarketData.marketSummariesForKeys([newMarketKey]))[0];

			const params = await perpsV2MarketData.parameters(newMarketKey); // pETH

			assert.equal(pETHSummary.market, sethMarket.address);
			assert.equal(pETHSummary.asset, newAssetKey);
			assert.equal(pETHSummary.maxLeverage, params.maxLeverage);
			const price = await sethMarket.assetPrice();
			assert.equal(pETHSummary.price, price.price);
			assert.equal(pETHSummary.marketSize, await sethMarket.marketSize());
			assert.equal(pETHSummary.marketSkew, await sethMarket.marketSkew());
			assert.equal(pETHSummary.currentFundingRate, await sethMarket.currentFundingRate());
			assert.equal(pETHSummary.currentFundingVelocity, await sethMarket.currentFundingVelocity());
			assert.equal(pETHSummary.feeRates.takerFee, params.takerFee);
			assert.equal(pETHSummary.feeRates.makerFee, params.makerFee);
			assert.equal(pETHSummary.feeRates.takerFeeDelayedOrder, params.takerFeeDelayedOrder);
			assert.equal(pETHSummary.feeRates.makerFeeDelayedOrder, params.makerFeeDelayedOrder);
			assert.equal(
				pETHSummary.feeRates.takerFeeOffchainDelayedOrder,
				params.takerFeeOffchainDelayedOrder
			);
			assert.equal(
				pETHSummary.feeRates.makerFeeOffchainDelayedOrder,
				params.makerFeeOffchainDelayedOrder
			);
		});
		...
		it('For market keys', async () => {
			...
			const summariesForAsset = await perpsV2MarketData.marketSummariesForKeys(
				['pBTC', 'pETH' + keySuffix].map(toBytes32)
			);
		...
		it('All summaries', async () => {
			...
			const pBTCSummary = summaries.find(summary => summary.key === toBytes32('pBTC'));
			const pETHSummary = summaries.find(summary => summary.key === toBytes32('pETH' + keySuffix));
			...
			assert.equal(pBTCSummary.market, perpsV2Market.address);
			assert.equal(pBTCSummary.asset, baseAsset);
			assert.equal(pBTCSummary.maxLeverage, fmParams.maxLeverage);
			let price = await perpsV2Market.assetPrice();
			assert.equal(pBTCSummary.price, price.price);
			assert.equal(pBTCSummary.marketSize, await perpsV2Market.marketSize());
			assert.equal(pBTCSummary.marketSkew, await perpsV2Market.marketSkew());
			assert.equal(pBTCSummary.currentFundingRate, await perpsV2Market.currentFundingRate());
			assert.equal(
				pBTCSummary.currentFundingVelocity,
				await perpsV2Market.currentFundingVelocity()
			);
			assert.equal(pBTCSummary.feeRates.takerFee, fmParams.takerFee);
			assert.equal(pBTCSummary.feeRates.makerFee, fmParams.makerFee);
			assert.equal(pBTCSummary.feeRates.takerFeeDelayedOrder, fmParams.takerFeeDelayedOrder);
			assert.equal(pBTCSummary.feeRates.makerFeeDelayedOrder, fmParams.makerFeeDelayedOrder);
			assert.equal(
				pBTCSummary.feeRates.takerFeeOffchainDelayedOrder,
				fmParams.takerFeeOffchainDelayedOrder
			);
			assert.equal(
				pBTCSummary.feeRates.makerFeeOffchainDelayedOrder,
				fmParams.makerFeeOffchainDelayedOrder
			);

			const pETHParams = await perpsV2MarketData.parameters(newMarketKey); // pETH

			assert.equal(pETHSummary.market, sethMarket.address);
			assert.equal(pETHSummary.asset, newAssetKey);
			assert.equal(pETHSummary.maxLeverage, pETHParams.maxLeverage);
			price = await sethMarket.assetPrice();
			assert.equal(pETHSummary.price, price.price);
			assert.equal(pETHSummary.marketSize, await sethMarket.marketSize());
			assert.equal(pETHSummary.marketSkew, await sethMarket.marketSkew());
			assert.equal(pETHSummary.currentFundingRate, await sethMarket.currentFundingRate());
			assert.equal(pETHSummary.feeRates.takerFee, pETHParams.takerFee);
			assert.equal(pETHSummary.feeRates.makerFee, pETHParams.makerFee);
			assert.equal(pETHSummary.feeRates.takerFeeDelayedOrder, pETHParams.takerFeeDelayedOrder);
			assert.equal(pETHSummary.feeRates.makerFeeDelayedOrder, pETHParams.makerFeeDelayedOrder);
			assert.equal(
				pETHSummary.feeRates.takerFeeOffchainDelayedOrder,
				pETHParams.takerFeeOffchainDelayedOrder
			);
			assert.equal(
				pETHSummary.feeRates.makerFeeOffchainDelayedOrder,
				pETHParams.makerFeeOffchainDelayedOrder
			);
		...
		it('All proxied market summaries', async () => {
			...
			const pBTCSummary = summaries.find(summary => summary.asset === toBytes32('pBTC'));
			const pETHSummary = summaries.find(summary => summary.asset === toBytes32('pETH'));
