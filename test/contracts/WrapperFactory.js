...
contract('WrapperFactory', async accounts => {
	const pynths = ['pUSD', 'pETH', 'ETH', 'PERI'];
	const [pETH, ETH] = ['pETH', 'ETH'].map(toBytes32);
	...
	describe('createWrapper', async () => {
		...
		describe('when successfully invoked', () => {
			...
			before(async () => {
				txn = await wrapperFactory.createWrapper(weth.address, pETH, toBytes32('PynthpETH'), {
					from: owner,
				});
			});
			...
			it('created wrapper has rebuilt cache', async () => {
				...
				// call totalIssuedPynths because it depends on address for ExchangeRates
				await etherWrapper.totalIssuedPynths();
			});
	...
	describe('distributeFees', async () => {
		...
		before(async () => {
			// deploy a wrapper
			const txn = await wrapperFactory.createWrapper(weth.address, pETH, toBytes32('PynthpETH'), {
				from: owner,
			});
		...
		it('issues pUSD to the feepool', async () => {
			...
			decodedEventEqual({
				event: 'Transfer',
				emittedFrom: await pUSDPynth.proxy(),
