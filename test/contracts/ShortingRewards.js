...
contract('ShortingRewards', accounts => {
	...
	before(async () => {
		...
		await Promise.all([
			...
			rewardsDistribution.setPeriFinanceProxy(rewardsToken.address, { from: owner }),
		...
		await short.addRewardsContracts(shortingRewards.address, pBTC, { from: owner });
	...
	beforeEach(async () => {
		...
		await issuepETHToAccount(toUnit(100), owner);

		await issuepUSDToAccount(toUnit(20000), account1);
		await issuepBTCtoAccount(toUnit(2), account1);
		await issuepETHToAccount(toUnit(10), account1);
	...
	describe('Pausable', async () => {
		...
		it('should revert calling enrol() when paused', async () => {
			await assert.revert(
				short.open(toUnit(15000), toUnit(1), pBTC, { from: account1 }),
		...
		it('should not revert calling stake() when unpaused', async () => {
			...
			await short.open(toUnit(15000), toUnit(1), pBTC, { from: account1 });
		});
	});
	...
	describe('rewardPerToken()', () => {
		...
		it('should be > 0', async () => {
			tx = await short.open(toUnit(20000), toUnit(1), pBTC, { from: account1 });
			id = await getid(tx);
	...
	describe('When positions are liquidated, they are withdraw from the rewards', () => {
		it('closing reduces the balance to 0', async () => {
			...
			tx = await short.open(toUnit(15000), toUnit(1), pBTC, { from: account1 });
			id = await getid(tx);
