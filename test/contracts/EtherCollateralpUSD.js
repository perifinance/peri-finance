...
contract('EtherCollateralpUSD', async accounts => {
	const MINUTE = 60;
	const DAY = 86400;
	const WEEK = 604800;
	const MONTH = 2629743;
	...
	const calculateLoanInterestFees = async (_address, _loanID) => {
		...
		const compoundedLoanAmount = pynthLoan.loanAmount.add(pynthLoan.accruedInterest);
		...
		const expectedInterest = interest.add(pynthLoan.accruedInterest);

		// console.log('expectedInterest', expectedInterest.toString());
		return expectedInterest;
	};

	const getPynthLoanTotalInterest = async (_address, _loanID) => {
		const pynthLoan = await etherCollateral.getLoan(_address, _loanID);
		return pynthLoan.accruedInterest;
	};
	...
	before(async () => {
		// Mock PERI, pUSD
		[{ token: pUSDPynth }] = await Promise.all([
			mockToken({ accounts, pynth: 'pUSD', name: 'Pynthetic USD', symbol: 'pUSD' }),
		]);

		({
			EtherCollateralpUSD: etherCollateral,
			FeePool: feePool,
		...
		// instruct the mock Issuer pynthsByAddress to return an address
		await mockGenericContractFnc({
			instance: mockIssuer,
			mock: 'Issuer',
			fncName: 'pynthsByAddress',
	...
	describe('when accessing the external views then', async () => {
		it('loanAmountFromCollateral should return 66.66 pUSD when opening a loan with 1 ETH @ 100', async () => {
		...
		it('loanAmountFromCollateral should return 10MM pUSD when opening a loan with 150000 ETH @ 100', async () => {
		...
		it('collateralAmountForLoan should return 1 ETH @ 100 required to open 66.66 pUSD', async () => {
		...
		it('collateralAmountForLoan should return 150000 ETH @ 100 required to open 10M pUSD', async () => {
	...
	describe('when opening a Loan', async () => {
		...
		describe('then revert when ', async () => {
			...
			it('attempting to issue more than the cap (issueLimit)', async () => {
				// limit pUSD supply cap to 100
				await etherCollateral.setIssueLimit(toUnit('100'), { from: owner });
				// 3 ETH will issue up to 200 pUSD
				await assert.revert(
		...
		describe('then create loan and', async () => {
			...
			const expectedpUSDLoanAmount = calculateLoanAmount(tenETH);
			const expectedMintingFee = calculateMintingFee(expectedpUSDLoanAmount);
			const expectedTotalLoan = expectedpUSDLoanAmount.add(expectedMintingFee);
			let openLoanAmount;
			...
			it('increase the totalIssuedPynths', async () => {
				assert.bnEqual(await etherCollateral.totalIssuedPynths(), expectedpUSDLoanAmount);
			});
			it('emit a LoanCreated event', async () => {
				assert.eventEqual(openLoanTransaction, 'LoanCreated', {
					account: address1,
					loanID: 1,
					amount: expectedpUSDLoanAmount,
				});
			});
			it('store the pynthLoan.acccount', async () => {
				const pynthLoan = await etherCollateral.getLoan(address1, loan1ID);
			...
			it('store the pynthLoan.loanAmount', async () => {
				const pynthLoan = await etherCollateral.getLoan(address1, loan1ID);
				assert.bnEqual(pynthLoan.loanAmount, expectedTotalLoan);
			});
			it('store the pynthLoan.loanID', async () => {
				const pynthLoan = await etherCollateral.getLoan(address1, loan1ID);
				assert.bnEqual(pynthLoan.loanID, loan1ID);
			});
			...
			it('add the loan issue amount minus minting fees to creators balance', async () => {
				const issuanceFee = calculateMintingFee(expectedpUSDLoanAmount);
				const pUSDBalance = await pUSDPynth.balanceOf(address1);
				assert.bnEqual(pUSDBalance, expectedpUSDLoanAmount.sub(issuanceFee));
			});
			...
			describe('then close the only loan open in the system', () => {
				...
				beforeEach(async () => {
					...
					await issuepUSDToAccount(toUnit('10000'), address1);
					// Go into the future
					...
					expectedInterestUSD = await getPynthLoanTotalInterest(address1, loan1ID);
				});
				...
				it('should reset the totalIssuedPynths to 0, ignoring accrued interest paid as fees', async () => {
					const totalIssuedPynthsAfter = await etherCollateral.totalIssuedPynths();

					assert.bnEqual(totalIssuedPynthsAfter, 0);
				});
			});
			...
			describe('when opening a second loan against address1', async () => {
				...
				const expectedpUSDLoanAmount = calculateLoanAmount(fiveThousandETH);
				const expectedMintingFee = calculateMintingFee(expectedpUSDLoanAmount);
				const expectedpUSDTotalLoan = expectedpUSDLoanAmount.add(expectedMintingFee);
				...
				it('then increase the totalIssuedPynths', async () => {
					assert.bnEqual(
						...
						totalIssuedPynthsBefore.add(expectedpUSDTotalLoan)
					);
				});
				...
				describe('when opening loan3 against address2', async () => {
					...
					const expectedpUSDLoanAmount = calculateLoanAmount(threeNintyETH);
					...
					it('then increase the totalIssuedPynths', async () => {
						assert.bnEqual(
							await etherCollateral.totalIssuedPynths(),
							totalSupplyBefore.add(expectedpUSDLoanAmount)
						);
					});
					...
					describe('when closing the first loan of address1', async () => {
						...
						describe('when closing the second loan of address1', async () => {
							...
							it('LoanClosed event emits the fees charged', async () => {
								...
								expectedInterestUSD = await getPynthLoanTotalInterest(address1, loan2ID);
							...
							describe('when closing loanID 3 from address2', async () => {
								// let calculatedFeepUSD;
								let closeLoanTransaction;

								beforeEach(async () => {
									// calculatedFeepUSD = await calculateLoanInterestFees(address2, loan3ID);
									// console.log('calculatedFeepUSD', calculatedFeepUSD.toString());

									// User will have had to made some positive trades to cover the interest
									await issuepUSDToAccount(toUnit('1000'), address2);
		...
		xdescribe('when creating multiple loans', () => {
			...
			beforeEach(async () => {
				// Issue pUSD to pay back interest
				await issuepUSDToAccount(toUnit('1000'), address1);
				await issuepUSDToAccount(toUnit('1000'), address2);
				await issuepUSDToAccount(toUnit('1000'), address3);
			});
			...
			it('then opening & closing from 10 different accounts', async () => {
				...
				for (let i = 0; i < first10Accounts.length; i++) {
					...
					// Issue pUSD to pay back interest
					await issuepUSDToAccount(toUnit('1000'), first10Accounts[i]);
				}
		...
		describe('when closing a Loan', async () => {
			...
			describe('check conditions', async () => {
				...
				it('when pUSD balance is less than loanAmount, then it reverts', async () => {
					// "Burn" some of accounts pUSD by sending to the owner
					await pUSDPynth.transfer(owner, toUnit('4'), { from: address1 });
				...
				describe('potential blocking conditions', () => {
					['System', 'Issuance'].forEach(section => {
						describe(`when ${section} is suspended`, () => {
							beforeEach(async () => {
								await issuepUSDToAccount(toUnit('100'), address1);
								await setStatus({ owner, systemStatus, section, suspend: true });
							});
					...
					describe('when rates have gone stale', () => {
						beforeEach(async () => {
							...
							issuepUSDToAccount(toUnit('100'), address1);
						});
						it('then calling closeLoan() reverts', async () => {
							await assert.revert(
								etherCollateral.closeLoan(loanID, {
						...
						describe('when pUSD gets a rate', () => {
							beforeEach(async () => {
								await updateRatesWithDefaults();
							});
							it('then calling closeLoan() succeeds', async () => {
			...
			describe('then it closes the loan and', async () => {
				...
				beforeEach(async () => {
					...
					feePoolBalanceBefore = await pUSDPynth.balanceOf(FEE_ADDRESS);
					// interestRatePerSec = await etherCollateral.interestPerSecond();
				...
				it('has the correct loanAmount', async () => {
					...
					const pynthLoan = await etherCollateral.getLoan(address1, openLoanID);
					assert.bnEqual(pynthLoan.loanAmount, expectedLoanAmount);
				});
				...
				it('reduce pUSD totalSupply', async () => {
				...
				it('emits a LoanClosed event', async () => {
					expectedInterestUSD = await getPynthLoanTotalInterest(address1, openLoanID);
					assert.eventEqual(closeLoanTransaction, 'LoanClosed', {
						account: address1,
						loanID: 1,
						feesPaid: expectedInterestUSD,
	...
	describe('when loanLiquidation is opened', async () => {
		...
		const expectedpUSDLoanAmount = calculateLoanAmount(oneETH);
		...
		beforeEach(async () => {
			...
			// Chad opens pUSD loan to liquidate Alice
		...
		xit('then alice has a pUSD loan balance', async () => {
			assert.bnEqual(await pUSDPynth.balanceOf(alice), expectedpUSDLoanAmount);
		});
		...
		describe('when loanLiquidation is open', () => {
			...
			describe('when chad has some pUSD and alice has her pUSD still', () => {
				beforeEach(async () => {
					// Chad has already opened a pUSD loan
				});
				describe('and chad liquidates alices pUSD loan for her ETH', async () => {
					...
					it('then alice pUSD balance is still intact', async () => {
						assert.ok(await pUSDPynth.balanceOf(alice), expectedpUSDLoanAmount);
					});
					it('then chads pUSD balance is 0 as it was burnt to repay the loan', async () => {
						assert.ok(await pUSDPynth.balanceOf(chad), 0);
					});
			...
			describe('when bob has some pUSD', () => {
				beforeEach(async () => {
					await pUSDPynth.transfer(bob, await pUSDPynth.balanceOf(alice), { from: alice });
					await issuepUSDToAccount(toUnit('100'), bob);
				});
				describe('and bob liquidates alices pUSD loan for her ETH', async () => {
					let liquidateLoanTransaction;
					beforeEach(async () => {
						liquidateLoanTransaction = await etherCollateral.liquidateUnclosedLoan(alice, loanID, {
							from: bob,
					...
					it('then alice pUSD balance is 0 (because she transfered it to bob)', async () => {
						assert.ok(await pUSDPynth.balanceOf(alice), 0);
					});
					it('then bobs pUSD balance is 0 as it was burnt to repay the loan', async () => {
						assert.ok(await pUSDPynth.balanceOf(bob), 0);
					});
					it('then emits a LoanLiquidated event', async () => {
						assert.eventsEqual(
							liquidateLoanTransaction,
