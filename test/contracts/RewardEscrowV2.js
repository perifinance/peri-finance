...
contract('RewardEscrowV2', async accounts => {
	...
	let rewardEscrowV2, periFinance, resolver;
	...
	before(async () => {
		({
			...
			accounts,
			contracts: ['RewardEscrowV2', 'MintablePeriFinance'],
		}));
		...
		// mint some peri into the contract (the holder of the PERI)
		await periFinance.mintSecondary(rewardEscrowV2.address, toUnit(100), {
			from: owner,
		});
		// create two entries
