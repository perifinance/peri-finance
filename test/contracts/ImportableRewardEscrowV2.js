...
contract('ImportableRewardEscrowV2', async accounts => {
	...
	let rewardEscrowV2, resolver, periFinance;
	...
	before(async () => {
		({
			...
			accounts,
			contracts: ['ImportableRewardEscrowV2', 'MintablePeriFinance'],
		}));
	});
	...
	describe('importVestingEntries', async () => {
		...
		describe('when called by the bridge', async () => {
			before(async () => {
				await resolver.importAddresses([toBytes32('PeriFinanceBridgeToBase')], [account1], {
					from: owner,
				});
			...
			describe('when balance is sufficient', async () => {
				before(async () => {
					await periFinance.mintSecondary(rewardEscrowV2.address, total, {
						from: account1,
					});
					await rewardEscrowV2.rebuildCache();
				});
