...
contract('BinaryOptionMarket @gas-skip', accounts => {
	...
	const setupNewMarket = async () => {
		...
		await exchangeRates.updateRates([pAUDKey], [toUnit(5)], await currentTime(), {
			from: oracle,
		});

		pUSDProxy = await pUSDPynth.proxy();

		await Promise.all([
			pUSDPynth.issue(initialBidder, pUSDQty),
			pUSDPynth.approve(manager.address, pUSDQty, { from: initialBidder }),
			pUSDPynth.issue(newBidder, pUSDQty),
			pUSDPynth.approve(manager.address, pUSDQty, { from: newBidder }),
		]);
		...
		await Promise.all([
			pUSDPynth.approve(market.address, pUSDQty, { from: initialBidder }),
			pUSDPynth.approve(market.address, pUSDQty, { from: newBidder }),
		]);
	...
	describe('Basic parameters', () => {
		it('static parameters are set properly', async () => {
			...
			assert.equal(oracleDetails.key, pAUDKey);
	...
	describe('Maturity condition resolution', () => {
		it('Current oracle price and timestamp are correct.', async () => {
			...
			await exchangeRates.updateRates([pAUDKey], [price], now, { from: oracle });
		...
		it('Result can fluctuate while unresolved, but is fixed after resolution.', async () => {
			...
			await exchangeRates.updateRates([pAUDKey], [initialStrikePrice.div(two)], now, {
				from: oracle,
			});
			assert.bnEqual(await market.result(), Side.Short);
			now = await currentTime();
			await exchangeRates.updateRates([pAUDKey], [initialStrikePrice.mul(two)], now, {
				from: oracle,
			});
		...
		it('A result equal to the strike price resolves long.', async () => {
			await fastForward(timeToMaturity + 1);
			const now = await currentTime();
			await exchangeRates.updateRates([pAUDKey], [initialStrikePrice], now, { from: oracle });
			await manager.resolveMarket(market.address);
			assert.isTrue(await market.resolved());
			assert.bnEqual(await market.result(), Side.Long);
		...
		it('Resolution can only occur once.', async () => {
			...
			await pUSDPynth.transfer(await mockManager.market(), toUnit(20));
		...
		it('Resolution properly remits the collected fees.', async () => {
			await fastForward(timeToMaturity + 1);
			await exchangeRates.updateRates([pAUDKey], [toUnit(0.7)], await currentTime(), {
				from: oracle,
			});
			...
			const [
				...
				pUSDPynth.balanceOf(initialBidder),
				pUSDPynth.balanceOf(feeAddress),
				market.deposited(),
			...
			const tx = await manager.resolveMarket(market.address);
			const logs = Pynth.decodeLogs(tx.receipt.rawLogs);
	...
	describe('Phases', () => {
		it('Can proceed through the phases properly.', async () => {
			...
			const now = await currentTime();
			await exchangeRates.updateRates([pAUDKey], [initialStrikePrice], now, {
				from: oracle,
			});
			await manager.resolveMarket(market.address);
	...
	describe('Bids', () => {
		...
		it('Bids withdraw the proper amount of pUSD', async () => {
			await market.bid(Side.Long, initialLongBid, { from: newBidder });
			await market.bid(Side.Short, initialShortBid, { from: newBidder });
			assert.bnEqual(
				await pUSDPynth.balanceOf(newBidder),
				pUSDQty.sub(initialLongBid.add(initialShortBid))
			);
		});

		it('Bids fail on insufficient pUSD balance.', async () => {
			await assert.revert(
		...
		it('Bids fail on insufficient pUSD allowance.', async () => {
			await pUSDPynth.approve(market.address, toBN(0), { from: newBidder });
			await assert.revert(
	...
	describe('Refunds', () => {
		...
		it('Refunds will fail if not enabled.', async () => {
			...
			await pUSDPynth.approve(localMarket.address, initialLongBid.mul(toBN(10)), {
				from: newBidder,
			});
		...
		it('Refunds remit the proper amount of pUSD', async () => {
			await market.bid(Side.Long, initialLongBid, { from: newBidder });
			await market.bid(Side.Short, initialShortBid, { from: newBidder });
			await market.refund(Side.Long, initialLongBid, { from: newBidder });
			await market.refund(Side.Short, initialShortBid, { from: newBidder });
			...
			const fee = multiplyDecimalRound(initialLongBid.add(initialShortBid), initialRefundFee);
			assert.bnEqual(await pUSDPynth.balanceOf(newBidder), pUSDQty.sub(fee));
		});
	...
	describe('Claiming Options', () => {
		it('Claims yield the proper balances before resolution.', async () => {
			await pUSDPynth.issue(pauper, pUSDQty);
			await pUSDPynth.approve(manager.address, pUSDQty, { from: pauper });
			await pUSDPynth.approve(market.address, pUSDQty, { from: pauper });
		...
		it('Can claim when the implicit losing claimable option balance is greater than the deposited pUSD', async () => {
			await pUSDPynth.issue(pauper, pUSDQty);
			await pUSDPynth.approve(manager.address, pUSDQty, { from: pauper });
			await pUSDPynth.approve(market.address, pUSDQty, { from: pauper });
	...
	describe('Exercising Options', () => {
		it('Exercising options yields the proper balances (long case).', async () => {
			...
			const newBidderBalance = await pUSDPynth.balanceOf(newBidder);
			const pauperBalance = await pUSDPynth.balanceOf(pauper);
			...
			assert.bnEqual(await long.bidOf(newBidder), toBN(0));
			assert.bnEqual(await short.bidOf(newBidder), toBN(0));
			assert.bnClose(await pUSDPynth.balanceOf(newBidder), newBidderBalance.add(longOptions), 1);

			assert.bnEqual(await long.balanceOf(pauper), toBN(0));
			assert.bnEqual(await short.balanceOf(pauper), toBN(0));
			assert.bnEqual(await long.bidOf(pauper), toBN(0));
			assert.bnEqual(await short.bidOf(pauper), toBN(0));
			assert.bnEqual(await pUSDPynth.balanceOf(pauper), pauperBalance);
		...
		it('Exercising options yields the proper balances (short case).', async () => {
			...
			assert.bnEqual(await long.bidOf(newBidder), toBN(0));
			assert.bnEqual(await short.bidOf(newBidder), toBN(0));
			assert.bnEqual(await pUSDPynth.balanceOf(newBidder), newBidderBalance);

			assert.bnEqual(await long.balanceOf(pauper), toBN(0));
			assert.bnEqual(await short.balanceOf(pauper), toBN(0));
			assert.bnEqual(await long.bidOf(pauper), toBN(0));
			assert.bnEqual(await short.bidOf(pauper), toBN(0));
			assert.bnClose(await pUSDPynth.balanceOf(pauper), pauperBalance.add(shortOptions), 1);
	...
	describe('Expiry', () => {
		it('Expired markets destroy themselves and their options.', async () => {
			...
			await exchangeRates.updateRates([pAUDKey], [initialStrikePrice], await currentTime(), {
				from: oracle,
			});
			await manager.resolveMarket(market.address);
		...
		it('Expired market remits any unclaimed options and extra pUSD to the caller.', async () => {
			await pUSDPynth.transfer(market.address, toUnit(1));
			const creatorBalance = await pUSDPynth.balanceOf(initialBidder);
			...
			const postCreatorBalance = await pUSDPynth.balanceOf(initialBidder);
