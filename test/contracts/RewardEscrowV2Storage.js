...
contract('RewardEscrowV2Storage', async accounts => {
	...
	let instance, resolver, frozenRewardEscrowV2, periFinance, firstNonFallbackId, user3NumEntries;
	...
	before(async () => {
		({
			...
			accounts,
			contracts: ['RewardEscrowV2Frozen', 'MintablePeriFinance'],
		}));

		// allow owner to write to create entries in old contract
		await resolver.importAddresses(
			['FeePool', 'PeriFinanceBridgeToBase', 'PeriFinanceBridgeToOptimism'].map(toBytes32),
			[owner, owner, owner],
			{ from: owner }
		);
		// and set RewardEscrowV2 in resolver to allow PERI transfers
		await resolver.importAddresses(
		...
		// mint some peri into it
		await periFinance.mintSecondary(frozenRewardEscrowV2.address, toUnit(1000), {
			from: owner,
		});
		...
		// set RewardEscrowV2 key to not the frozen rewards to prevent any PERI transfers by it
	...
	describe('public mutative methods on frozen contract', async () => {
		...
		it('are broken due to inability to transfer PERI', async () => {
