	...
	describe('getPynth()', () => {
		describe('when a mock for Issuer is added', () => {
			...
			beforeEach(async () => {
				...
				// now instruct the mock Issuer that pynths(any) must return "account4"
			...
			it('when getPynth() is invoked', async () => {
				const pynth = await resolver.getPynth(toBytes32('pUSD'));
				assert.equal(pynth, account4);
			});
		});
		describe('when a PeriFinance is created with a few added pynths', () => {
			let pUSDContract;
			beforeEach(async () => {
				({ PynthpUSD: pUSDContract } = await setupAllContracts({
					accounts,
					existing: {
						AddressResolver: resolver,
					},
					pynths: ['pUSD'],
					contracts: ['PeriFinance', 'StakingStateUSDC'],
				}));
			});
