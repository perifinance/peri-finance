...
contract('OneNetAggregators', async accounts => {
	...
	let addressResolver, aggregatorDebtRatio, aggregatorIssuedPynths;

	let mockPeriFinanceDebtShare, mockIssuer;
	...
	before(async () => {
		...
		aggregatorIssuedPynths = await setupContract({
			accounts,
			args: [addressResolver.address],
		...
		mockPeriFinanceDebtShare = await smock.fake('PeriFinanceDebtShare');

		mockIssuer.totalIssuedPynths.returns(ethers.utils.parseEther('500'));
		mockPeriFinanceDebtShare.totalSupply.returns(ethers.utils.parseEther('1000'));

		await addressResolver.importAddresses(
			[toBytes32('Issuer'), toBytes32('PeriFinanceDebtShare')],
			[mockIssuer.address, mockPeriFinanceDebtShare.address]
		);
	});
