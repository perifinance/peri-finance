...
module.exports = {
	...
	async updateRatesWithDefaults({ exchangeRates, circuitBreaker, owner, debtCache }) {
		const keys = ['PERI', 'pAUD', 'pEUR', 'pBTC', 'iBTC', 'pETH', 'ETH'].map(toBytes32);
		const rates = ['0.1', '0.5', '1.25', '5000', '4000', '172', '172'].map(toUnit);
		// set up any missing aggregators
		await setupMissingPriceAggregators(exchangeRates, owner, keys);
	...
	// Helper function that can issue pynths directly to a user without having to have them exchange anything
	async issuePynthsToUser({ owner, issuer, addressResolver, pynthContract, user, amount }) {
		// First override the resolver to make it seem the owner is the PeriFinance contract
		await addressResolver.importAddresses(['Issuer'].map(toBytes32), [owner], {
			from: owner,
		});
		// now have the pynth resync its cache
		await pynthContract.rebuildCache();

		await pynthContract.issue(user, amount, {
			from: owner,
		});
	...
	async setExchangeFeeRateForPynths({ owner, systemSettings, pynthKeys, exchangeFeeRates }) {
		await systemSettings.setExchangeFeeRateForPynths(pynthKeys, exchangeFeeRates, {
			from: owner,
		});
	},
	...
	async setStatus({
		...
		} else if (section === 'PynthExchange') {
			if (suspend) {
				await systemStatus.suspendPynthExchange(pynth, reason, { from: owner });
			} else {
				await systemStatus.resumePynthExchange(pynth, { from: owner });
			}
		} else if (section === 'Pynth') {
			if (suspend) {
				await systemStatus.suspendPynth(pynth, reason, { from: owner });
			} else {
				await systemStatus.resumePynth(pynth, { from: owner });
			}
		} else {
	...
	async prepareSmocks({ accounts = [], contracts, mocks = {} }) {
		for (const [i, contract] of Object.entries(contracts).concat([
			...
			//console.log(label, mocks[label].address);
		}
