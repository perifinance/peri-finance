...
contract('BaseRewardEscrowV2', async accounts => {
	...
	beforeEach(async () => {
		({ mocks, resolver } = await prepareSmocks({
			contracts: ['FeePool', 'Issuer', 'PeriFinance'],
	...
	describe('Creating vesting Schedule', async () => {
		describe('When appending vesting entry via feePool', async () => {
			...
			it('should revert appending a vesting entry from account1', async () => {
				...
				mocks['PeriFinance'].smocked.balanceOf.will.return.with(parseEther('10'));
			...
			it('should revert appending a vesting entry with a zero amount', async () => {
				// Transfer of PERI to the escrow must occur before creating an entry
				mocks['PeriFinance'].smocked.balanceOf.will.return.with(parseEther('1'));
			...
			it('should revert appending a vesting entry if there is not enough PERI in the contracts balance', async () => {
				// Transfer of PERI to the escrow must occur before creating an entry
				mocks['PeriFinance'].smocked.balanceOf.will.return.with(parseEther('1'));
			...
			describe('When successfully appending new escrow entry for account 1 with 10 PERI', () => {
				...
				it('Account 1 should have balance of 10 PERI', async () => {
					assert.bnEqual(await baseRewardEscrowV2.balanceOf(account1), escrowAmount);
				});
				it('totalEscrowedBalance of the contract should be 10 PERI', async () => {
				...
				describe('When 6 months has passed', () => {
					...
					it('then the vesting entry has 0 peri claimable', async () => {
	...
	describe('Creating a new escrow entry by approval', async () => {
		...
		beforeEach(async () => {
			// approve rewardEscrow to spend PERI
			mocks['PeriFinance'].smocked.allowance.will.return.with(parseEther('10'));

			// stub transferFrom
			mocks['PeriFinance'].smocked.transferFrom.will.return.with(true);
		...
		describe('when successfully creating a new escrow entry for acount 1', () => {
			...
			it('Account1 should have balance of 10 PERI', async () => {
				assert.bnEqual(await baseRewardEscrowV2.balanceOf(account1), escrowAmount);
			});
			it('Account1 should have totalVested Account Balance of 0', async () => {
				assert.bnEqual(await baseRewardEscrowV2.totalVestedAccountBalance(account1), new BN(0));
	...
	describe('Vesting', () => {
		...
		describe('Vesting of vesting entry after 6 months (before escrow ends)', () => {
			...
			let escrowAmount, timeElapsed, entryID, claimablePERI;
			beforeEach(async () => {
			...
			it('should vest 0 amount if entryID does not exist for user', async () => {
				...
				// Check user has no vested PERI
				assert.bnEqual(await mockedPeriFinance.balanceOf(account1), toUnit('0'));

				// Check rewardEscrow does not have any PERI
				assert.bnEqual(await mockedPeriFinance.balanceOf(baseRewardEscrowV2.address), escrowAmount);
			...
			it('should vest and transfer 0 PERI from contract to the user', async () => {
				claimablePERI = await baseRewardEscrowV2.getVestingEntryClaimable(account1, entryID);

				const escrowBalanceBefore = await mockedPeriFinance.balanceOf(baseRewardEscrowV2.address);
				...
				// Check user has the 0 vested PERI
				assert.bnEqual(await mockedPeriFinance.balanceOf(account1), 0);

				// Check rewardEscrow contract has same amount of PERI
				assert.bnEqual(
					await mockedPeriFinance.balanceOf(baseRewardEscrowV2.address),
					escrowBalanceBefore
				);
				...
				// claimablePERI is 0
				assert.bnEqual(claimablePERI, 0);
		...
		describe('When vesting after escrow ended', () => {
			...
			it('should vest and transfer all the peri to the user', async () => {
				await baseRewardEscrowV2.vest([entryID], {
					from: account1,
				});

				// Check user has all their vested PERI
				assert.bnEqual(await mockedPeriFinance.balanceOf(account1), escrowAmount);

				// Check rewardEscrow does not have any PERI
				assert.bnEqual(await mockedPeriFinance.balanceOf(baseRewardEscrowV2.address), toUnit('0'));
			});
		...
		describe('Vesting multiple vesting entries', () => {
			...
			describe('When another user (account 1) vests all their entries', () => {
				it('should vest all entries and transfer peri to the user', async () => {
					await baseRewardEscrowV2.vest([entryID1, entryID2, entryID3], {
						from: account2,
					});

					// Check account1 has no PERI in their balance
					assert.bnEqual(await mockedPeriFinance.balanceOf(account1), toUnit('0'));

					// Check account2 has no PERI in their balance
					assert.bnEqual(await mockedPeriFinance.balanceOf(account2), toUnit('0'));

					// Check rewardEscrow has all the PERI
					assert.bnEqual(
						await mockedPeriFinance.balanceOf(baseRewardEscrowV2.address),
						toUnit('1000')
					);
			...
			it('should vest all entries and transfer peri from contract to the user', async () => {
				await baseRewardEscrowV2.vest([entryID1, entryID2, entryID3], {
					from: account1,
				});

				// Check user has all their vested PERI
				assert.bnEqual(await mockedPeriFinance.balanceOf(account1), toUnit('1000'));
			...
			it('should vest all entries and ignore duplicate attempts to vest same entries again', async () => {
				...
				// Check user has same amount of PERI
				assert.bnEqual(await mockedPeriFinance.balanceOf(account1), toUnit('1000'));
		...
		describe('Vesting multiple vesting entries with different duration / end time', () => {
			...
			describe('when the first two entrys are vestable', () => {
				...
				it('should vest only first 2 entries and transfer peri from contract to the user', async () => {
					await baseRewardEscrowV2.vest([entryID1, entryID2, entryID3], {
						from: account1,
					});
					...
					assert.bnEqual(
						await mockedPeriFinance.balanceOf(account1),
				...
				it('should vest and update totalEscrowedAccountBalance', async () => {
					...
					// This account should have any 500 PERI escrowed
					escrowedAccountBalance = await baseRewardEscrowV2.totalEscrowedAccountBalance(account1);
				...
				it('should vest entryID1 and entryID2 and ignore duplicate attempts to vest same entries again', async () => {
					...
					// Check user have vested escrowAmount1 and escrowAmount2 PERI
					assert.bnEqual(
						await mockedPeriFinance.balanceOf(account1),
						escrowAmount1.add(escrowAmount2)
					);

					// Check rewardEscrow does has escrowAmount3 PERI
					assert.bnEqual(
						await mockedPeriFinance.balanceOf(baseRewardEscrowV2.address),
						escrowAmount3
					);
					...
					// Check rewardEscrow has same escrowAmount3 PERI
					assert.bnEqual(
						await mockedPeriFinance.balanceOf(baseRewardEscrowV2.address),
						escrowAmount3
					);
	...
	describe('Read Vesting Schedule', () => {
		...
		beforeEach(async () => {
			// Transfer of PERI to the escrow must occur before creating a vestinng entry
			mocks['PeriFinance'].smocked.balanceOf.will.return.with(parseEther('1000'));
