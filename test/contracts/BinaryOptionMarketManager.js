...
contract('BinaryOptionMarketManager @gas-skip', accounts => {
	...
	let manager, factory, systemStatus, exchangeRates, addressResolver, pUSDPynth, oracle;
	...
	before(async () => {
		...
		await Promise.all([
			pUSDPynth.issue(initialCreator, pUSDQty),
			pUSDPynth.approve(manager.address, pUSDQty, { from: initialCreator }),
			pUSDPynth.issue(bidder, pUSDQty),
			pUSDPynth.approve(manager.address, pUSDQty, { from: bidder }),
		]);
	});
	...
	describe('Market creation', () => {
		...
		it('Cannot create a market without sufficient capital to cover the initial bids.', async () => {
			...
			await pUSDPynth.issue(dummy, pUSDQty);
			...
			await pUSDPynth.approve(manager.address, pUSDQty, { from: dummy });
	...
	describe('Market tracking', () => {
		it('Multiple markets can exist simultaneously, and debt is tracked properly across them.', async () => {
			...
			await Promise.all(
				markets.map(market => pUSDPynth.approve(market.address, pUSDQty, { from: bidder }))
			);
			...
			await fastForward(expiryDuration + 1000);
			await exchangeRates.updateRates([pAUDKey], [toUnit(2)], await currentTime(), {
				from: oracle,
			});
	...
	describe('Deposit management', () => {
		...
		it('Bidding affects total deposits properly.', async () => {
			...
			await pUSDPynth.issue(bidder, pUSDQty);
			await pUSDPynth.approve(market.address, pUSDQty, { from: bidder });
	...
	describe('Market migration', () => {
		...
		before(async () => {
			...
			await Promise.all(
				markets.map(m => pUSDPynth.approve(m.address, toUnit(1000), { from: bidder }))
			);
			await pUSDPynth.approve(newManager.address, toUnit(1000), { from: bidder });
		...
		it('Markets can be migrated to a manager with existing markets.', async () => {
			...
			await pUSDPynth.approve(newManager.address, toUnit('1000'));
			const now = await currentTime();
