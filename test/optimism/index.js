...
const { itCanPerformPynthExchange } = require('./pynthExchange.test');
...
describe('Layer 2 production tests', () => {
	let PeriFinanceL1, PeriFinanceL2;
	...
	describe('when instances have been deployed in local L1 and L2 chains', () => {
		before('connect to contracts', async () => {
			PeriFinanceL1 = connectContract({ contract: 'PeriFinance', provider: this.providerL1 });
		...
		before('simulate exchange rates and debt cache', async () => {
			async function simulateExchangeRates({ provider, owner, useOvm }) {
				...
				currencyKeys = currencyKeys.filter(key => key !== toBytes32('pUSD'));
				const additionalKeys = ['PERI', 'ETH'].map(toBytes32); // The Depot uses the key "ETH" as opposed to "pETH" for its ether price
		...
		describe('[GENERAL] properties', () => {
			it('shows the expected owners', async () => {
				assert.equal(await PeriFinanceL1.owner(), this.ownerAddress);
				assert.equal(await PeriFinanceL2.owner(), this.ownerAddress);
			});

			it('shows the instances have the expected total supplies', async () => {
				assert.bnGte(await PeriFinanceL1.totalSupply(), ethers.utils.parseEther('0'));
				assert.bnGte(await PeriFinanceL2.totalSupply(), ethers.utils.parseEther('0'));
			});
		});
		...
		itCanPerformPynthExchange({ ctx: this });
	});
});
