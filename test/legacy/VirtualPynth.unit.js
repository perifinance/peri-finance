...
contract('VirtualPynth unit tests', async accounts => {
	...
	describe('with common setup', () => {
		...
		const behaviors = require('./VirtualPynth.behaviors').call(this, { accounts });

		describe('constructor', () => {
			const amount = '1001';
			behaviors.whenInstantiated({ amount, user: owner, pynth: 'pBTC' }, () => {
				it('then each constructor arg is set correctly', async () => {
					assert.equal(trimUtf8EscapeChars(await this.instance.name()), 'Virtual Pynth pBTC');
					assert.equal(trimUtf8EscapeChars(await this.instance.symbol()), 'vpBTC');
		...
		describe('secsLeftInWaitingPeriod()', () => {
			behaviors.whenInstantiated({ amount: '1000', user: owner, pynth: 'pBTC' }, () => {
		...
		describe('readyToSettle()', () => {
			behaviors.whenInstantiated({ amount: '999', user: owner, pynth: 'pBTC' }, () => {
				behaviors.whenMockedWithMaxSecsLeft({ maxSecsLeft: 100 }, () => {
					...
					describe('settle()', () => {
						behaviors.whenSettlementCalled({ user: owner }, () => {
							it('then Exchanger.settle() is invoked with the correct params', async () => {
								...
								assert.equal(this.mocks.Exchanger.smocked.settle.calls[0][1], toBytes32('pBTC'));
							});
							...
							it('then the balance of the users vPynth is 0', async () => {
								assert.equal(await this.instance.balanceOf(owner), '0');
							});
							it('then the user is transferred the balance of the pynth', async () => {
								assert.equal(await this.mocks.Pynth.smocked.transfer.calls.length, 1);
								assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][0], owner);
								assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][1], amount);
							});
							behaviors.whenSettlementCalled({ user: owner }, () => {
						...
						behaviors.whenMockedSettlementOwing({ reclaim: 333 }, () => {
							behaviors.whenSettlementCalled({ user: owner }, () => {
								it('then the user is transferred the remaining balance of the pynths', async () => {
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls.length, 1);
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][0], owner);
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][1], '666');
								});
							});
						});
						...
						behaviors.whenMockedSettlementOwing({ rebate: 1 }, () => {
							behaviors.whenSettlementCalled({ user: owner }, () => {
								it('then the user is transferred the entire balance of the pynths', async () => {
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls.length, 1);
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][0], owner);
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][1], '1000');
								});
							});
						});
						...
						behaviors.whenUserTransfersAwayTokens({ amount: '666', from: owner }, () => {
							behaviors.whenSettlementCalled({ user: owner }, () => {
								it('then the user is transferred their portion balance of the pynths', async () => {
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls.length, 1);
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][0], owner);
									assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][1], '333');
								});
							});

							behaviors.whenMockedSettlementOwing({ reclaim: 300 }, () => {
								// total pynths is 999 - 300 = 699. User has 1/3 of the vPynth supply
								behaviors.whenSettlementCalled({ user: owner }, () => {
									it('then the user is transferred their portion balance of the pynths', async () => {
										assert.equal(await this.mocks.Pynth.smocked.transfer.calls.length, 1);
										assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][0], owner);
										assert.equal(await this.mocks.Pynth.smocked.transfer.calls[0][1], '233');
									});
								});
							});
						});
