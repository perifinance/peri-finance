...
contract('RewardEscrowV2', async accounts => {
	...
	let rewardEscrowV2, periFinance, feePool, rewardEscrow, resolver;
	...
	before(async () => {
		...
		await rewardEscrow.setPeriFinance(periFinance.address, { from: owner });
	...
	describe('migrateVestingSchedule', () => {
		...
		describe('when migrating pending vesting entries and entries that can be vested already', () => {
			...
			beforeEach(async () => {
				...
				await periFinance.transfer(rewardEscrow.address, toUnit('2000'), { from: owner });

				// migrateAccountEscrowBalance to RewardEscrowV2 for 1040 PERI
			...
			it('should migrate 52 pending vesting entry schedule from old rewardEscrouw', async () => {
				...
				// There would be another 520 PERI to migrate as a single vestable entry
				...
				for (let id = 1; id <= 52; id++) {
					...
					// check all entries have 10 PERI
		...
		describe('when migrating pending vesting entries but less than 52 entries', () => {
			...
			beforeEach(async () => {
				...
				// migrateAccountEscrowBalance to RewardEscrowV2 for 2000 PERI
				await rewardEscrowV2.migrateAccountEscrowBalances([account1], [toUnit('2000')], [0], {
					from: owner,
				});

				// append vesting entries for account1 - 4 vesting entries of 500 PERI
				for (let i = 0; i < 4; i++) {
					await rewardEscrow.appendVestingEntry(account1, quantity, { from: feePoolAccount });
			...
			it('should migrate 4 pending vesting entry schedule from old rewardEscrouw', async () => {
				...
				for (let id = 1; id <= 4; id++) {
					...
					// check all entries have 500 PERI
					assert.bnEqual(entry.escrowAmount, quantity);
				}
