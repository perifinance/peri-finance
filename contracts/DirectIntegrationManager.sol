 ...
 * https://sips.periFinance.io/sips/sip-267/
 *
 * Used by the Spartan Council to approve an external contract, (i.e. one which is not owned or managed by the PeriFinance protocol),
 * to interact with PeriFinance's core exchange functionalities with overridden parameters.
...
contract DirectIntegrationManager is Owned, MixinSystemSettings, IDirectIntegrationManager {
    ...
    /* ========== MUTATIVE FUNCTIONS ========== */
    /**
     * Sets an override to be used for a given direct integration that supersedes the default PeriFinance parameter value.
     * @param integration the address of the external integrator's contract
     ...
     * external interactions with PeriFinance contracts and the parameter overrides that would be implemented.
     */
