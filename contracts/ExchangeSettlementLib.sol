...
library ExchangeSettlementLib {
        ...
        )
    {
        ...
        // by checking a reclaim or refund we also check that the currency key is still a valid pynth,
        // as the deviation check will return 0 if the pynth has been removed.
        if (updateCache && (reclaimed > 0 || refunded > 0)) {
            bytes32[] memory key = new bytes32[](1);
            key[0] = currencyKey;
            resolvedAddresses.debtCache.updateCachedPynthDebts(key);
        }
    ...
    function _reclaim(
        ...
        resolvedAddresses.issuer.pynths(currencyKey).burn(from, amount);
        IPeriFinanceInternal(address(resolvedAddresses.periFinance)).emitExchangeReclaim(from, currencyKey, amount);
    }
    ...
    function _refund(
        ...
        resolvedAddresses.issuer.pynths(currencyKey).issue(from, amount);
        IPeriFinanceInternal(address(resolvedAddresses.periFinance)).emitExchangeRebate(from, currencyKey, amount);
    }
    ...
    // Internal function to aggregate each individual rebate and reclaim entry for a pynth
