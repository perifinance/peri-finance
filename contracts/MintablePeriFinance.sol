...
contract MintablePeriFinance is BasePeriFinance {
    ...
    function onlyAllowFromBridge() internal view {
        require(msg.sender == periFinanceBridge(), "Can only be invoked by bridge");
    }
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[0] = CONTRACT_PERIFINANCE_BRIDGE;
        addresses = combineArrays(existingAddresses, newAddresses);
    }

    function periFinanceBridge() internal view returns (address) {
        return requireAndGetAddress(CONTRACT_PERIFINANCE_BRIDGE);
    }
