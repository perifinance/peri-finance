...
// https://docs.periFinance.io/contracts/source/contracts/exchangerateswithdexpricing
contract ExchangeRatesWithDexPricing is ExchangeRates {
        ...
        )
    {
        ...
        IDirectIntegrationManager.ParameterIntegrationSettings memory usdSettings =
            directIntegrationManager().getExchangeParameters(msg.sender, pUSD);
        ...
        )
    {
        ...
        // outlined here: https://sips.periFinance.io/sips/sip-198/#computation-methodology-in-atomic-pricing
    ...
    /// @dev One of `sourceCurrencyKey` or `destCurrencyKey` should be pUSD. There are two parameters to indicate directionality. Because this function returns "price", if the source is pUSD, the result will be flipped.
    ...
    function _getPriceFromDexAggregator(
        ...
        require(
            sourceSettings.currencyKey == pUSD || destinationSettings.currencyKey == pUSD,
            "Atomic swaps must go through pUSD"
        );
        ...
        return destinationSettings.currencyKey == "pUSD" ? result : SafeDecimalMath.unit().divideDecimalRound(result);
    }
    ...
    function pynthTooVolatileForAtomicExchange(bytes32 currencyKey) public view returns (bool) {
        ...
        return pynthTooVolatileForAtomicExchange(settings);
    }

    function pynthTooVolatileForAtomicExchange(IDirectIntegrationManager.ParameterIntegrationSettings memory settings)
        public
        view
        returns (bool)
    {
        // pUSD is a special case and is never volatile
        if (settings.currencyKey == "pUSD") return false;
