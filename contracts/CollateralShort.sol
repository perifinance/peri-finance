...
contract CollateralShort is Collateral {
    ...
    function deposit(
        ...
        require(amount <= IERC20(address(_pynthpUSD())).allowance(msg.sender, address(this)), "Allowance too low");
    ...
    function withdraw(uint id, uint amount) external returns (uint principal, uint collateral) {
        ...
        IERC20(address(_pynthpUSD())).transfer(msg.sender, amount);
    }
    ...
    function _repayWithCollateral(
        ...
        // 3. Get the equivalent payment amount in pUSD, and also distinguish
        // the fee that would be charged for both principal and interest.
        (uint expectedAmount, uint exchangeFee, ) = _exchanger().getAmountsForExchange(payment, loan.currency, pUSD);
        uint paymentSUSD = expectedAmount.add(exchangeFee);

        // 4. Reduce the collateral by the equivalent (total) payment amount in pUSD,
        ...
        // 5. Pay exchange fees.
        _payFees(exchangeFee, pUSD);

        // 6. Burn pUSD held in the contract.
        _pynthpUSD().burn(address(this), collateralToRemove);
